...
class PrettyPrinter(gast.NodeVisitor):
  ...
  def generic_visit(self, node, name=None):
    ...
    if name:
      self. print('%s%s=%s%s' % (self._indent(), self._field(name),
                                 self._type(node), cont))
    else:
      self. print('%s%s%s' % (self._indent(), self._type(node), cont))
    ...
    for f in node._fields:
      ...
      if isinstance(v, list):
        if v:
          self. print('%s%s=[' % (self._indent(), self._field(f)))
          self.indent_lvl += 1
          for n in v:
            self.generic_visit(n)
          self.indent_lvl -= 1
          self. print('%s]' % (self._indent()))
        else:
          self. print('%s%s=[]' % (self._indent(), self._field(f)))
      elif isinstance(v, gast.AST):
        self.generic_visit(v, f)
      elif isinstance(v, str):
        self. print('%s%s=%s' % (self._indent(), self._field(f),
                                 self._value('"%s"' % v)))
      else:
        self. print('%s%s=%s' % (self._indent(), self._field(f),
                                 self._value(v)))
